...
export function OperationalTable({ actions }: { actions: FieldAction[] }) {
  return (
    <div className="overflow-hidden rounded-[20px] border border-border">
      <div className="hidden grid-cols-[0.8fr_1fr_1.6fr_1fr_0.8fr] bg-card-soft px-4 py-2.5 text-[11px] font-semibold uppercase tracking-wider text-muted-text/80 md:grid">
        <span>Rep</span>
        <span>Territory</span>
        <span>Action</span>
        <span>Due date</span>
      ...
      <div className="divide-y divide-border bg-card">
        {actions.map((action) => (
          <div key={action.action_id} className="grid gap-3 px-4 py-3 text-sm transition-colors hover:bg-card-soft md:grid-cols-[0.8fr_1fr_1.6fr_1fr_0.8fr] md:items-center">
            <div>
