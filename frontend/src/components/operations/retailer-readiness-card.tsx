...
export function RetailerReadinessCard({ alerts }: { alerts: InventoryAlert[] }) {
  ...
  return (
    <DashboardCard>
      ...
      <div className="mt-3 flex items-center gap-2 rounded-[18px] border border-field/20 bg-field/10 px-4 py-3 text-xs font-medium text-[#237143]">
        <PackageCheck className="h-4 w-4" />
        Deployment allowed: retailer stock exceeds territory threshold.
      </div>
      <div className="mt-3 flex items-center gap-2 rounded-[18px] border border-border bg-white px-4 py-3 text-xs font-medium text-muted">
...
function ReadinessMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[16px] border border-border bg-card-soft px-3 py-2">
